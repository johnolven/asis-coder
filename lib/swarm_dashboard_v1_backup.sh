...
draw_devices() {
    ...
    if [ -z "$devices_json" ] || [ "$devices_json" = "null" ]; then
        tput_color yellow
        echo "│ No devices file found at $SWARM_DEVICES_FILE"
        tput_reset
        return
    fi

    local count
    count="$(echo "$devices_json" | jq 'length' 2>/dev/null || echo 0)"
    ...
    while IFS='|' read -r name ip user; do
        ...
        # Get stats if online
        local cpu="?" ram="?/?GB" agents="0"
        if [ "$status" = "online" ]; then
            local stats
            stats="$(ssh -o ConnectTimeout=1 -o StrictHostKeyChecking=no "$user@$ip" \
                "top -bn1 | grep 'Cpu(s)' | awk '{print \$2}' | cut -d'%' -f1; \
                 free -m | awk '/^Mem:/ {printf \"%.1f/%.1fG\", \$3/1024, \$2/1024}'; \
                 tmux ls 2>/dev/null | grep -c ralph" 2>/dev/null)"
            if [ $? -eq 0 ]; then
                cpu="$(echo "$stats" | sed -n '1p')%"
                ram="$(echo "$stats" | sed -n '2p')"
                agents="$(echo "$stats" | sed -n '3p')"
